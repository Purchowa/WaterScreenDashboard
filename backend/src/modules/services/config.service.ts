...
export default class ConfigService {
    public getAllConfig = async (): Promise<ConfigModelType | undefined> => {
        const config = await ConfigModel.findOne({}).select('-_id -__v -picture._id -wasRead -workRange._id');
        return config ? config : undefined;
    }

    public getWaterscreenConfig = async (): Promise<ConfigModelType | undefined> => {
        const config = await ConfigModel.findOne({}).select('-_id -__v -mailList -picture._id -workRange._id');
        if (!config)
            return undefined;
